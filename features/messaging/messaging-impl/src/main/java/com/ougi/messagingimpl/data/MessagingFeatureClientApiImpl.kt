...
import com.ougi.messagingimpl.data.workmanager.MessageReceiverWorker
...
class MessagingFeatureClientApiImpl @Inject constructor(private val context: Context) :
    ...
    override fun startMessagingWork(expedited: Boolean) {
        val constraints = Constraints.Builder()
            ...
            .build()
        val workRequest = OneTimeWorkRequestBuilder<MessageReceiverWorker>()
            .apply {
                addTag(MessageReceiverWorker.WORK_NAME)
    ...
    override fun observeState(onStateChanged: (WebSocketState) -> Unit) {
        observeReceivingWork { values ->
            val state = values[MessageReceiverWorker.STATE] as WebSocketState
    ...
    override fun sendMessage(message: String) {
        observeReceivingWork { values ->
            val webSocket = values[MessageReceiverWorker.WEB_SOCKET] as WebSocket
            webSocket.send(message)
        }
    }

    private fun observeReceivingWork(onProgressChanged: (Map<String, Any>) -> Unit) {
        workManager.getWorkInfosByTagLiveData(MessageReceiverWorker.WORK_NAME)
